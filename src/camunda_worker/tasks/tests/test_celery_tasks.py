...
class RouteTaskTests(TestCase):
    ...
    def test_task_fetch_and_lock(self, m_test_execute):
        ...
        with patch(
            "camunda_worker.tasks.tasks.celery.fetch_and_lock",
    ...
    @patch("camunda_worker.tasks.tasks.celery.complete")
    @patch("camunda_worker.tasks.tasks.celery.execute")
    ...
    @patch("camunda_worker.tasks.tasks.celery.complete")
    @patch("camunda_worker.tasks.tasks.celery.execute", side_effect=Exception)
    ...
    @patch("camunda_worker.tasks.tasks.celery.complete", side_effect=Exception)
    @patch("camunda_worker.tasks.tasks.celery.execute")
